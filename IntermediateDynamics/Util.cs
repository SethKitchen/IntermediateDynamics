...
namespace IntermediateDynamics
{
    public class Util
    {
        public static double SolveForVariable(double lower_bound, double max_search_value, string variable, double rhsAns, Expr toSolve)
        {
            for (double i = lower_bound + 0.001; i <= max_search_value; i += 0.001)
            {
                ...
                if (Math.Abs(ans.RealValue - rhsAns) < .01)
                {
                    return i;
                }
            }
